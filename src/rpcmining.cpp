...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "NoLimitCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "NoLimitCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "NoLimitCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "NoLimitCoin is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH (CTransaction& tx, pblock->vtx)
    {
        ...
        {
            entry.push_back(Pair("fee", (int64_t)(tx.GetValueIn(mapInputs, true) - tx.GetValueOut())));
