...
void Shutdown(void* parg)
{
    ...
    RenameThread("NoLimitCoin-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("NoLimitCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  NoLimitCoind [options]                     " + "\n" +
                  "  NoLimitCoind [options] <command> [params]  " + _("Send command to -server or NoLimitCoind") + "\n" +
                  "  NoLimitCoind [options] help                " + _("List commands") + "\n" +
                  "  NoLimitCoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "NoLimitCoin:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("NoLimitCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("NoLimitCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: NoLimitCoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: NoLimitCoind.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 6521 or testnet: 16521)") + "\n" +
        ...
        "  -maxreceivebuffer=<n>  " + _("Maximum per-connection receive buffer, <n>*1000 bytes (default: 5000)") + "\n" +
            ...
            "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 6520 or testnet: 16520)") + "\n" +
...
bool AppInit2()
{
    ...
    #define PROCESS_DEP_ENABLE 0x00000001
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  NoLimitCoin is probably already running."), strDataDir.c_str()));
        ...
        if (GetBoolArg("-shrinkdebugfile", !fDebug))
            ShrinkDebugFile();
        printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
        printf("NoLimitCoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
        ...
        if (fDaemon)
            fprintf(stdout, "NoLimitCoin server starting\n");
        ...
        if (filesystem::exists(GetDataDir() / strWalletFileName))
        {
            ...
            if (r == CDBEnv::RECOVER_OK)
            {
                ...
                uiInterface.ThreadSafeMessageBox(msg, _("NoLimitCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
            }
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of NoLimitCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart NoLimitCoin to complete") << "\n";
